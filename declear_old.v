...
`define VAWidth 7 ///APP 地址宽度，2^7 = 128，对应 APP RAM 深度。
`define VRWidth 8 //可能是 APP 或 QSN 累加值的内部更宽位宽，用于避免溢出。
`define PM  32 //并行度参数。
`define b  5 //可能是 "bitwidth for CN/VN index"（check/variable node 索引的宽度），因为 2^5=32，正好和 PM=32 对应。
`define P_num  27 //APP_RAM个数
`define inWidth  32
`define inNum 32 //输入数据宽度 32bit，输入通道数 32。
`define AWidth 8 //累加器宽度，用于在 CN 更新时存储和累加。
...
`define APPdata_Len `inNum*`VWidth
...
`define HijWidth 13 //每个 H 矩阵元素的存储宽度。存 shift 值 (0 ~ Zc-1)。 因为 Zc=4096，log2(4096)=12，所以用 13 bit 存储移位量。
`define maxIterNum 3 // number of iterations 
`define APP_addr_width 8
`define APP_addr_max 8'd128
`define APP_addr_rd_max 7'd127

`define HROM_cols 26 //H 矩阵基图每行的最大列数 = 26。
`define HROM_Width `HijWidth*`HROM_cols //每行存储宽度。

`define Zc 4096 //lifting size，LDPC 码扩展因子。
`define APPRam_depth 128 //APP RAM 的深度 = 128，存放 APP 输入 LLR。 128*32 
