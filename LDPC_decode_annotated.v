...
always @(*)
begin
	if(buffer_valid_D0 && group_to_buffer == 0)
	begin
...
// =============================================================================
 ...
 // =============================================================================
// APPmsg_ini_addr increases when buffer_valid is high
always @(posedge clk or negedge rst_n)
begin
	if(!rst_n)
	begin
...
always @(posedge clk or negedge rst_n) //layernum和iternum 分别控制
begin
	if(!rst_n)begin
		iternum <= 0;
	end
...
begin
	...
	begin
		...
		decode_valid <= APP_addr_rd_end_D1;
	end
end
...
begin
	...
	else if(APP_rd_endD13)  //控制 update_end 对 Layernum 计数
	begin
		update_end <= 1;
	end
	else
...
HROM1_0 u10_HRom(.addra(shift_addr_rd1[1:0]),.clka(clk),.douta(bg1_shift_0),.ena(HROM_rd_en)); //现在矩阵只有4行	
...
begin
	...
	begin
		APP_wr_en_D0 <= APP_decodin_wr_en;
	end
end
